		...
		<div class="fill_message">
			<p>
				...
				<select id="address" onchange="changeAddress(this.value)">
					...
					<c:forEach items="${sessionScope.list }" var="addr">
						<option value="${addr.id }" <c:if test="${addr.id == sessionScope.addressId}">selected</c:if> >${addr.local}</option>
					</c:forEach>					
				</select>
				<script type="text/javascript">
					function changeAddress(addressId){
						window.location.href="${pageContext.request.contextPath}/address/selectAddresses?addressId="+addressId;
					}
				</script>
			</p>
			<form name="ctl00" method="post" action="${pageContext.request.contextPath }/order/addOrder" id="f">
				...
				<table class="tab_login">
					...
					<tr>
						...
						<td>
							<input type="text" class="text_input" name="zipCode" id="postalCode" value="${address.zipCode }"/>
				...
				<div class="login_in">
					<a href="${pageContext.request.contextPath }/front/order/order_info.jsp">
						<input id="btnClientRegister" class="button_1" type="button" value="取消" />
					</a>			
