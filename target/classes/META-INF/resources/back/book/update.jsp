	...
	<form action="${pageContext.request.contextPath}/book/updateBook" method="post" enctype="multipart/form-data">
			<table>
				...
				<tr>
					<td>所属分类：</td>
					<td>
						<select name="category.id" class="el-select__inner inner2">
							<c:forEach items="${seconds}" var="category">
								<option value="${category.id }" <c:if test="${book.category.id == category.id }">selected</c:if>  >${category.name }</option>
							</c:forEach>
						</select>
					</td>
					<td>字数：</td>
					<td><input type="number" name="wordNum" class="el-input__inner" value="${book.wordNum }" ></td>
				</tr>
				...
				<tr>
					...
					<td>编辑推荐：</td>
					<td rowspan="2"><textarea class="el-textarea__inner" name="editorRecommend">${book.editorRecommend }</textarea></td>
				</tr>
				<tr>
					<td>出版社：</td>
				...
				<tr>
					...
					<td>内容简介：</td>
					<td rowspan="2"><textarea class="el-textarea__inner" name="contentAbstract" >${book.contentAbstract }</textarea></td>
				</tr>
				<tr>
					<td>版次：</td>
				...
				<tr>
					<td>印刷时间：</td>
					<td><input type="date" name="printDate" class="el-input__inner" value="<fmt:formatDate value='${book.printDate }' pattern='yyyy-MM-dd' />"></td>
					<td>作者简介：</td>
					<td rowspan="2"><textarea class="el-textarea__inner" name="authorAbstract">${book.authorAbstract }</textarea></td>
				</tr>
				<tr>
					<td>印次：</td>
				...
				<tr>
					...
					<td>媒体评论：</td>
					<td rowspan="2"><textarea class="el-textarea__inner" name="mediaCommentary">${book.mediaCommentary }</textarea></td>
				</tr>
				<tr>
					<td>包装：</td>
